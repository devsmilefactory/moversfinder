...
const DocumentsPage = () => {
  ...
  const { documents, loading, loadDocuments, uploadDocument } = useDriverStore();
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadDocument(user.id, selectedDocumentType.id, uploadFile, expiryDate || null);

      // Reset and close
      setUploadFile(null);
      setExpiryDate('');
      setShowUploadModal(false);
      alert(`${selectedDocumentType.name} uploaded successfully! It will be reviewed by admin.`);
    } catch (error) {
  ...
  const completionPercentage = Math.round((documents.filter(doc => doc.status === 'approved').length / DOCUMENT_TYPES.length) * 100);
