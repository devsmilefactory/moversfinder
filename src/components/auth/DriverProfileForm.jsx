...
const DriverProfileForm = ({ onComplete }) => {
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      // License plate is required for conflict checking
  ...
  const saveProgress = async () => {
    ...
    try {
      ...
      // Prepare partial profile data (only fields that have been filled)
      const profileData = {
        user_id: user.id,
        submission_status: 'draft', // Mark as draft when saving progress
        updated_at: new Date().toISOString(),
      };

      // Add fields based on completed steps
      if (currentStep >= 1 && formData.license_plate) {
        profileData.license_plate = formData.license_plate;
        // Note: operator_id will be added once database column exists
      }
      ...
      if (currentStep >= 2) {
        ...
        if (formData.national_id) profileData.national_id = formData.national_id;
      }

      if (currentStep >= 3) {
        if (formData.license_number) profileData.license_number = formData.license_number;
        if (formData.license_expiry) profileData.license_expiry = formData.license_expiry;
        if (formData.license_class) profileData.license_class = formData.license_class;
      }

      if (currentStep >= 4) {
        if (formData.vehicle_make) profileData.vehicle_make = formData.vehicle_make;
        if (formData.vehicle_model) profileData.vehicle_model = formData.vehicle_model;
        if (formData.vehicle_year) profileData.vehicle_year = formData.vehicle_year;
        if (formData.vehicle_color) profileData.vehicle_color = formData.vehicle_color;
      }

      if (currentStep >= 5) {
        if (formData.bank_name) profileData.bank_name = formData.bank_name;
        if (formData.account_number) profileData.account_number = formData.account_number;
        // Note: account_holder_name will be added once database column exists
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const profileData = {
        ...
        submission_status: 'submitted', // Mark as submitted for verification
      ...
      // driver_profiles already updated above; no base profiles completion updates
            ...
            <div className="text-center space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-left">
                ...
                <ul className="text-sm text-blue-700 space-y-1">
                  <li>✓ Your progress has been saved securely</li>
                  <li>✓ You can continue from where you left off</li>
                  <li>✓ Complete all steps to submit for verification</li>
                </ul>
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 m-4">
            <div className="text-center space-y-4">
              ...
              <div className="space-y-2">
                <p className="text-gray-600">
                  Your driver profile has been submitted for verification.
                </p>
                <p className="text-sm text-gray-500">
                  {formData.operator_id
                    ? 'You will be notified once your operator and BMTOA approve your profile. This typically takes 24-48 hours.'
                    : 'You will be notified once BMTOA approves your profile. This typically takes 24-48 hours.'
                  }
                </p>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-left">
                ...
                <ul className="text-sm text-blue-700 space-y-1">
                  <li>✓ Your profile is now under review</li>
                  <li>✓ You'll receive an email notification when approved</li>
                  <li>✓ Once approved, you can start accepting ride requests</li>
                  <li>✓ You can check your approval status in your dashboard</li>
                </ul>
              </div>
              ...
              <Button
                onClick={onComplete}
                className="w-full bg-[#334155] hover:bg-[#FFC107] hover:text-[#334155]"
              >
                Go to Dashboard
