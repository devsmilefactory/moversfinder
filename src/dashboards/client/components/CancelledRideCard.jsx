...
const CancelledRideCard = ({ ride, onClick, tabContext = 'cancelled' }) => {
  ...
  const errandSummary = useMemo(() => {
    ...
    const info = summarizeErrandTasks(ride.errand_tasks || ride.tasks);
  ...
  return (
    ...
    >
      ...
      {/* Errand summary */}
      {errandSummary && (
        <div className="mb-3 bg-green-50 rounded-lg px-3 py-2 border border-green-200 opacity-75">
          <p className="text-sm font-semibold text-green-800">Errand Tasks Planned</p>
          <p className="text-xs text-green-700">
            {errandSummary.completed}/{errandSummary.total} tasks completed before cancellation
          </p>
        </div>
      )}
