...
import { createSingleRideBooking } from '../../../services/corporateBookingService';

/**
 * Corporate Single Ride Booking Form
 *
 * For booking rides for individual passengers
 * Includes passenger selection and ride details
 * Now with confirmation step before submission
 */

const CorporateSingleRideForm = ({ onBack, onComplete, initialLocation }) => {
  ...
  const [passengerSelection, setPassengerSelection] = useState('existing'); // 'existing' or 'other'
  const [selectedPassenger, setSelectedPassenger] = useState('');
  const [passengerType, setPassengerType] = useState('staff'); // 'staff' or 'clients'

  // Manual passenger details (for 'other' option)
  ...
  const handleConfirm = async (bookingDataWithSaveOptions) => {
    try {
      const result = await createSingleRideBooking(bookingDataWithSaveOptions, user);

      if (result.success) {
        alert(`✅ Ride booked successfully!${bookingDataWithSaveOptions.saveOptions.saveTrip ? '\n💾 Trip saved for future use!' : ''}`);
        onComplete();
      } else {
  ...
  const getBookingData = () => {
    return {
      ...
      serviceType: 'rides',
      passengerSelection,
      selectedPassenger: passengerSelection === 'existing' ? selectedPassenger : null,
      manualPassenger: passengerSelection === 'other' ? manualPassenger : null,
      scheduleType,
  ...
  const filteredPassengers = passengers.filter(p =>
    passengerSelection === 'existing' && p.type === passengerType
  );
  ...
  if (currentStep === 'confirmation') {
    return (
      <TripConfirmationStep
        ...
        showSavePassenger={passengerSelection === 'other'}
      />
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-700">Book Ride</h1>
          <p className="text-slate-600">Single Booking</p>
        </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="bg-white rounded-xl shadow-lg p-8 space-y-6">
          ...
          <div className="pb-6 border-b border-slate-200">
            <h2 className="text-lg font-semibold text-slate-700 mb-4">Who are you booking for?</h2>
            
            <div className="grid grid-cols-2 gap-4 mb-4">
              <button
                type="button"
                onClick={() => setPassengerSelection('existing')}
                className={`p-4 rounded-lg border-2 transition-all ${
                  passengerSelection === 'existing'
                    ? 'border-yellow-400 bg-yellow-50'
                    : 'border-slate-200 hover:border-slate-300'
                }`}
              >
                <div className="text-2xl mb-2">👤</div>
                <div className="font-semibold text-slate-700">Existing Passenger</div>
                <div className="text-sm text-slate-600">Select from staff or clients</div>
              </button>
              
              <button
                type="button"
                onClick={() => setPassengerSelection('other')}
                className={`p-4 rounded-lg border-2 transition-all ${
                  passengerSelection === 'other'
                    ? 'border-yellow-400 bg-yellow-50'
                    : 'border-slate-200 hover:border-slate-300'
                }`}
              >
                ...
                <div className="text-sm text-slate-600">Enter details manually</div>
              </button>
            </div>
            ...
            {passengerSelection === 'existing' && (
              <div className="space-y-4">
                <div className="flex gap-4">
                  <button
                    type="button"
                    onClick={() => setPassengerType('staff')}
                    className={`flex-1 py-2 px-4 rounded-lg border-2 transition-all ${
                      passengerType === 'staff'
                        ? 'border-yellow-400 bg-yellow-50 text-slate-700 font-semibold'
                  ...
                  <button
                    type="button"
                    onClick={() => setPassengerType('clients')}
                    className={`flex-1 py-2 px-4 rounded-lg border-2 transition-all ${
                ...
                <FormSelect
                  ...
                  options={filteredPassengers.map(p => ({
                    value: p.id,
            ...
            {passengerSelection === 'other' && (
              <div className="grid grid-cols-3 gap-4">
                <FormInput
                  ...
                  onChange={(e) => setManualPassenger({ ...manualPassenger, name: e.target.value })}
                ...
                <FormInput
                  ...
                  name="phone"
                  type="tel"
                  value={manualPassenger.phone}
                  onChange={(e) => setManualPassenger({ ...manualPassenger, phone: e.target.value })}
                  placeholder="+263..."
                  required
                />
                <FormInput
...
export default CorporateSingleRideForm;
