-- Fix: accept_offer_atomic inserts into driver_locations without coordinates
-- This caused: null value in column "coordinates" of relation "driver_locations" violates not null constraint
--
-- Approach:
-- - Recreate the RPC with a safe UPSERT that always supplies coordinates on INSERT.
-- - Prefer existing driver_locations.coordinates if present; otherwise fallback to a GeoJSON Point (0,0).
--
-- Note: This does NOT enable scheduling/recurring features; instant-only flow remains intact.
...
DECLARE
  ...
  v_existing_driver_coords jsonb;
BEGIN
  -- Lock offer
  SELECT * INTO v_offer
  ...
  -- Get any existing driver coordinates (preferred for accuracy).
  -- If no row exists, fall back to a safe GeoJSON Point to satisfy NOT NULL.
  SELECT dl.coordinates
  INTO v_existing_driver_coords
  FROM public.driver_locations dl
  WHERE dl.driver_id = v_offer.driver_id;

  -- Mark driver engaged (ensure coordinates is never NULL on INSERT)
  ...
  VALUES (
    ...
    now(),
    coalesce(
      v_existing_driver_coords,
      jsonb_build_object('type', 'Point', 'coordinates', jsonb_build_array(0, 0))
    )
  )
